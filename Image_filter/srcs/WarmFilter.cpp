...
void WarmFilter::apply(Image* image) {
    ...
    float redValue[] = { 0,80,160,256 };
    float blueValue[] = { 0,50,100,256 };
    ...
    merge(channels, img_warm);




    imshow("image", img);
    imshow("result", img_warm);

    waitKey(0);

    image->setImage(img_warm);
}
